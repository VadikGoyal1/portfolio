...
export default function ParticleBackground({ theme }: ParticleBackgroundProps) {
  ...
  useEffect(() => {
    ...
    function animate() {
      ...
      particles.forEach((particle, index) => {
        ...
        ctx.fillStyle = theme.text.includes("white")
        ...
        if (window.innerWidth >= 768) {
          particles.slice(index + 1).forEach((otherParticle) => {
            ...
            if (distance < 80) {
              ...
              ctx.strokeStyle = theme.text.includes("white")
  ...
  return <canvas ref={canvasRef} className="absolute inset-0 pointer-events-none opacity-20 sm:opacity-30" />
}
